...
function AdminRecycleBin() {
    ...
    const fetchDeleted = async () => {
        try {
            setLoading(true);
            const res = await axios.get("http://localhost:5000/api/students/recyclebin");
    ...
    const handleRestore = async (id) => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/students/restore/${id}`);
    ...
    const handlePermanentDelete = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/students/permanent/${id}`);
                                    ...
                                    return (
                                        <tr key={student._id}>
                                            <td>
                                                {student.passport ? (
                                                    <img
                                                        src={`https://localhost:5000/uploads/${student.passport}`}
                                                        alt="Passport"
                                                        style={passportImg}
                                                    />
                                                ) : (
